...
#Creamos en directorio en donde se almacenarÃ¡ el archivo de configuraciÃ³n y log de odoo:
sudo mkdir /etc/odoo && sudo mkdir /var/log/odoo
# Agregamos los repositorios APT de postgres
echo deb http://apt.postgresql.org/pub/repos/apt/ xenial-pgdg main >> /etc/apt/sources.list.d/pgdg.list
wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add -
sudo apt-get update
# Instalamos esta librerÃ­a aparte, ya que se pone mariquita si la instalamos junto a las demÃ¡s
sudo apt-get install libsasl2-dev
# Instalamos Postgres y librerÃ­as base del sistema:
...
#Damos permiso al directorio que contiene los archivos de OdooERPÂ  e instalamos las dependencias de python3:
...
#Agregamos los siguientes parÃ¡metros al archivo de configuraciÃ³n de odoo:
...
# Para que postgres se inicie automaticamente
